...
export default function QrResultsDisplay({ qrCodes, onBack, settings }: QrResultsDisplayProps) {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      // Helper function to sanitize filename - keep dots and dashes like the original URL
      // Example: https://qr.novax-intl.com/70888001 → https---qr.novax-intl.com-70888001
      const sanitizeFilename = (text: string) => {
        return text
          .replace(/:\/\//g, '___TRIPLEDASH___')  // protect :// 
          .replace(/[^a-zA-Z0-9._\-]/g, '-')  // replace special chars with dash
          .replace(/-+/g, '-')  // collapse consecutive dashes
          .replace(/^-|-$/g, '')  // trim leading/trailing dashes
          .replace(/___TRIPLEDASH___/g, '---')  // restore triple dash
      }

      // Helper function to extract number from QR data for VDP number
      const extractNumber = (data: string): string => {
        // Try to extract any number from the data
        const match = data.match(/(\d+)/)
        return match ? match[1] : String(Math.floor(Math.random() * 1000000))
      }
      ...
      // Create CSV file with QR code data - VDP_NUMBER and Filename columns
      const csvHeader = "QR Code Data,Filename,VDP_NUMBER\n"
      const csvRows = qrCodes
        .map((qr, index) => {
          if (qr.success) {
            const vdpNumber = extractNumber(qr.data)
            const sanitizedName = sanitizeFilename(qr.data)
            const filename = `${sanitizedName}.${extension}`
            return `"${qr.data.replace(/"/g, '""')}",${filename},${vdpNumber}`
          }
          return ""
        })
        .filter((row) => row !== "")
  ...
  const handleDownloadSingle = (qr: QrResult) => {
    if (qr.qrDataUrl) {
      const sanitizeFilename = (text: string) => {
        return text
          .replace(/:\/\//g, '___TRIPLEDASH___')
          .replace(/[^a-zA-Z0-9._\-]/g, '-')
          .replace(/-+/g, '-')
          .replace(/^-|-$/g, '')
          .replace(/___TRIPLEDASH___/g, '---')
      }
      const extension = settings.outputFormat === "JPG" ? "jpg" : settings.outputFormat === "SVG" ? "svg" : "png"
      const a = document.createElement("a")
      a.href = qr.qrDataUrl
      a.download = `${sanitizeFilename(qr.data)}.${extension}`
      a.click()
    }
  }
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
          {qrCodes.map((qr) => (
            <div key={qr.id} className="flex flex-col">
              ...
              <p className="text-xs text-muted-foreground truncate mt-1 text-center w-full">{qr.data.slice(0, 20)}</p>
            </div>
          ))}
        </div>
