...
import { PRICING_TIERS, formatNumber, calculateCreditsPerDollar, type PricingTier, type Currency, convertToMYR, formatCurrency } from "@/lib/credits"
...
export default function PricingCards({ onPurchaseComplete }: PricingCardsProps) {
  ...
  const handleSelectTier = async (tier: PricingTier) => {
    try {
      ...
      const actualPrice = currency === 'MYR' ? convertToMYR(tier.price) : tier.price
    ...
    <>
      <div className="flex justify-center mb-6">
        <div className="inline-flex rounded-lg border p-1 bg-muted">
          ...
          <Button
            variant={currency === 'MYR' ? 'default' : 'ghost'}
            size="sm"
            ...
            >
              ...
              <CardHeader className="pb-4">
                ...
                <CardDescription>
                  <div className="text-3xl font-bold text-foreground mt-2">
                    {currency === 'MYR' ? formatCurrency(convertToMYR(tier.price), 'MYR') : formatCurrency(tier.price, 'USD')}
                  </div>
                  <div className="text-xs text-muted-foreground mt-0.5">
                    {currency === 'MYR' && `≈ ${formatCurrency(tier.price, 'USD')} USD`}
                    {currency === 'USD' && `≈ ${formatCurrency(convertToMYR(tier.price), 'MYR')} MYR`}
                  </div>
              ...
              <CardContent className="flex-grow">
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-2">
                    <Check className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                    <span className="text-sm">Get 100 FREE credits every month</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <Check className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
